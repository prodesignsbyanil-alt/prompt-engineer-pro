...
The prompt should be tracing-friendly, vector-ready, unique, and describe composition, shapes, and elements clearly. No meta lines like numbering or char counts.`.trim();
        ...
        if (provider === "ChatGPT") {
          ...
          const res = await fetch("/api/gemini", {
            ...
            body: JSON.stringify({ prompt: userPrompt, key }),
          });
          const data = await res.json();
  ...
  function resetAll() {
    ...
    setTotalCount(10);
    setColorCount(2);
    setStyles({ minimalist: true, silhouette: true, flatColor: true, blackWhite: true, noGradient: true, withoutTypography: true });
    setPrompts([]);
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <header className="sticky top-0 z-50 border-b border-zinc-200/70 dark:border-zinc-800/70 bg-white/80 dark:bg-zinc-900/80 backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="inline-flex items-center gap-2 rounded-xl border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
              {theme === "dark" ? <Sun className="h-4 w-4"/> : <Moon className="h-4 w-4"/>}
              <span>{theme === "dark" ? "Light Mode" : "Dark Mode"}</span>
            </button>
            {!isAuthed ? (
              <button onClick={() => setLoginOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white px-3 py-2 text-sm transition-colors">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { setEmail(""); setPrompts([]); toastPush({ type: "neutral", title: "Logged out", desc: "Session cleared." }); }} className="inline-flex items-center gap-2 rounded-xl border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800">
      ...
      <main className={clsx("mx-auto max-w-7xl px-4 py-6 grid gap-6", !isAuthed && "opacity-50 saturate-50 pointer-events-none select-none") }>
        <section className="grid gap-3 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <div className="grid gap-1.5">
              ...
              <select className="rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent px-3 py-2 text-sm" value={provider} onChange={(e) => setProvider(e.target.value)}>
            ...
            <div className="grid gap-1.5 md:col-span-2">
              ...
              <div className="flex items-center gap-2">
                <input type={apiKeyVisible ? "text" : "password"} className="flex-1 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent px-3 py-2 text-sm" placeholder="Enter API key" value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)} />
                <button onClick={() => setApiKeyVisible(v => !v)} className="inline-flex items-center justify-center rounded-xl border border-zinc-300 dark:border-zinc-700 px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800">{apiKeyVisible ? <EyeOff className="h-4 w-4"/> : <Eye className="h-4 w-4"/>}</button>
                <button onClick={handleSaveKey} className={clsx("inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-white transition-colors", savePulse ? "bg-emerald-600" : "bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800")}><Save className="h-4 w-4"/> Save</button>
              </div>
        ...
        <section className="grid gap-3 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              ...
              <input type="text" className="rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent px-3 py-2 text-sm" placeholder="Characters and Scenes — e.g., ‘smiling pumpkins’" value={subject} onChange={(e) => setSubject(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
        ...
        <section className="grid gap-3 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
          ...
          {prompts.length === 0 ? (
            <p className="text-sm text-zinc-500 dark:text-zinc-400">No prompts yet. Configure settings and click <span className="font-semibold">Generate Prompts</span>.</p>
          ) : (
      ...
      {loginOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-2xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
            ...
            <input type="email" placeholder="name@gmail.com" className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent px-3 py-2 text-sm" onKeyDown={(e) => { if (e.key === 'Enter') loginNow(); }} id="pep-login-email" />
